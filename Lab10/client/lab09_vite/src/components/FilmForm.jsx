...
const FilmForm = (props) => {
   ...
   */
  //console.log(`Film in FilmForm ${props.film.title}`);
  const [title, setTitle] = useState(props.film ? props.film.title : '');
  const [favorite, setFavorite] = useState(props.film ? props.film.fav : false);
  const [watchDate, setWatchDate] = useState((props.film && props.film.date) ? dayjs(props.film.date).format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD'));
  const [rating, setRating] = useState(props.film ? props.film.score : 0);
  ...
  const handleSubmit = (event) => {
    ...
    const film = {"title": title.trim(), "fav": favorite, "score": rating, "user" : 1};
    if(watchDate != "")  // adding watchDate only if it is defined
      film.date = dayjs(watchDate);
    else
      film.date = null 
