    ...
    return new Promise((resolve, reject) => {
        ...
        db.all(sql, (err, rows) => {
            if (err)
                ...
                const films = rows.map((f) => new Film(f.id,f.title,f.favorite,f.rating,1,f.watchdate));
                resolve(films);
            }
        });
    });
...
function addFilm(film){
    return new Promise((resolve,reject) => {
        ...
        db.run(sql,[film.title,film.fav,film.date,film.score],(err) => {
            if (err){
...
function updateFilm(id, info){
    return new Promise((resolve,reject) => {
        ...
        db.run(sql,[info.title,info.fav,info.date,info.score,info.user,id],
            (err) => {
                if (err){
